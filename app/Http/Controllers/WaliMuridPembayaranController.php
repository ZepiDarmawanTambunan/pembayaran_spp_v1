...
class WaliMuridPembayaranController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $data['tagihan'] = Tagihan::waliAnanda()
        ->findOrFail($request->tagihan_id);
        $data['model'] = new Pembayaran();
        $data['method'] = 'POST';
        $data['route'] = 'wali.pembayaran.store';
        $data['listRekeningYayasan'] = RekeningYayasan::pluck('nama_bank', 'id');
        $data['listBank'] = Bank::pluck('nama_bank', 'id');
        $data['url'] = route('wali.pembayaran.create', [
            'tagihan_id' => $request->tagihan_id,
        ]);

        if($request->rekening_yayasan_id != ''){
            $data['bankYangDipilih'] = RekeningYayasan::findOrFail($request->rekening_yayasan_id);
        }
    ...
    public function store(Request $request)
    {
        ...
        if($request->filled('pilihan_bank')){
            ...
            $request->validate([
                'rekening_yayasan_id' => 'required|exists:rekening_yayasans,id',
            ...
            $dataPembayaran = [
                'rekening_yayasan_id' => $request->rekening_yayasan_id,
